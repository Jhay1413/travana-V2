...
5. **Page-level mini-features.** ✅ **RESOLVED — keep as-is.** `pages/` is the route layer and stays populated by design (30 top-level route files + page-local subfolders like `agency/` 31, `hr-v2/` 15, `portal/` 12). Their components are largely route-local (used by a single screen), so they stay in `pages/` rather than being force-extracted into `features/`. The cross-cutting layers (api/hooks/types/reusable components) are already in `features/`. If a specific page-local component later becomes reused across routes, move just that component into the owning `features/<x>/components` at that point — no bulk extraction.
