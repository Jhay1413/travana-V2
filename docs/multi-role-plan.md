...
- `user.orgRole` — single varchar per user (loose enum), set when they accept an invite.
- `user.role` — legacy text column; drives the frontend permissions matrix at [client/src/lib/permissions.ts](../client/src/lib/permissions.ts). `Admin` (= org_admin) **already has full `"admin"` permission** on clients/quotes/bookings/enquiries.
- `branch_members.orgRole` — role within a branch the user belongs to. `orgBranchScope` already reads from `branch_members` first, falling back to `user.orgRole`.
...
**So the actual feature is: let a user opt into additional roles, and surface the corresponding UI.**
...
- `user.orgRole` becomes a **derived "primary role"** = the highest-power role in the set (used by legacy code and as the default landing experience). Recomputed whenever the role set changes.
...
-- Backfill from existing user.orgRole. Skips platform_admin, NULL orgRole, and
-- users with no org. Idempotent on re-run.
...
Whenever the role set changes, also write the primary role to legacy `user.orgRole` (and the matching legacy `user.role` string used by [client/src/lib/permissions.ts](../client/src/lib/permissions.ts)).
...
Add `orgRoles: string[]` next to the existing `orgRole`. `resolveOrgAndBranchForUser` reads the junction table for the user and builds the union.
...
- `POST   /api/v2/auth/self/roles/sell` (opt-in)   — self-service: any org_admin can grant themselves the `agent` role on themselves

Every mutation:
1. Validates role compatibility (referral_agent exclusion).
2. Writes `user.role.add` / `user.role.remove` audit rows via the existing `admin_audit_log`.
3. Updates the legacy `user.orgRole` to the new primary.
4. Updates `user.role` (legacy enum) to the matching casing for the new primary.
...
On `/agency/profile` (or a small new `/agency/me` page), an org_admin sees a toggle:

```
[✓] Also work as a sales agent
    Adds Pipeline, Enquiries, Quotes, Bookings, and Clients to your menu.
```
...
- Tests: add a role → verify nav now shows extra sections, primary role didn't change, legacy `user.orgRole` was recalculated to highest-rank.
...
4. **branch_members** — when an org_admin opts into `agent`, do we **also** add them to a branch (e.g. the default branch) via `branch_members`, or do they stay branch-less? (My recommendation: stay branch-less. Their deals carry `branch_id` from the picker per the earlier conversation; they're not a branch member.)
5. **Manager → sell?** — should this also enable a `branch_manager` to add `agent`? They technically already write_all in permissions, but the *targets/leaderboard* picture differs. (My recommendation: yes, same self-service toggle, scoped to "anyone with at least branch_manager rank".)
6. **Legacy `user.role` text column** — keep recomputing it from primary, or freeze it at signup and stop using it? Frontend's [permissions.ts](../client/src/lib/permissions.ts) still drives off it. (My recommendation: keep recomputing it from primary for v1; deprecate in a future cleanup.)
...
3. **Existing invite flow** writes a single `orgRole` to `user`. After PR A, the invite acceptance should *also* insert into `user_org_roles`. Add a small step inside `inviteService.acceptInvite`.
...
8. **Performance** — `findByUserAndOrg` is a single indexed lookup per request. Negligible.
...
- One new table, one migration. Backward-compatible everywhere via primary-role derivation.
