...
import { hrApi, type DocumentCategory, type EmployeeRow as ApiEmployeeRow } from "@/features/hr/api/hr.api";
...
const DOCUMENT_CATEGORIES: { id: DocumentCategory; label: string; hint: string }[] = [
  { id: "Contract",      label: "Contract",      hint: "Signed employment agreement" },
  { id: "NDA",           label: "NDA",           hint: "Non-disclosure agreement" },
  { id: "Right to Work", label: "Right to Work", hint: "Passport / visa / ID verification" },
  { id: "Policies",      label: "Policies",      hint: "Acknowledged handbooks and policies" },
  { id: "Training",      label: "Training",      hint: "Certificates and learning records" },
  { id: "Other",         label: "Other",         hint: "Anything that doesn't fit the slots above" },
];
...
function DocumentsTab({ employee }: { employee: Employee }) {
  ...
  const grouped = useMemo(() => {
    const map = new Map<DocumentCategory, typeof employee.documents>();
    for (const cat of DOCUMENT_CATEGORIES) map.set(cat.id, []);
    for (const d of employee.documents) {
      const list = map.get(d.category as DocumentCategory) ?? [];
      list.push(d);
      map.set(d.category as DocumentCategory, list);
    }
    return map;
  }, [employee]);

  const triggerCategoryUpload = (category: DocumentCategory) => {
  ...
  const handleFiles = async (files: FileList | null) => {
    ...
    for (const file of fileArray) {
      await new Promise<void>((resolve) =>
        upload.mutate(
          { userId: employee.id, file, category: category ?? undefined },
          {
            onSuccess: () => {
              toast({
                title: category ? `${category} document attached` : "Uploaded",
    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between gap-3">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-0.5">
            Upload into the matching category so the onboarding checklist updates · PDF, image, Word, Excel · up to 10MB
          </p>
        </div>
      </div>
      ...
      <div className="space-y-4">
        {DOCUMENT_CATEGORIES.map((cat) => {
          ...
          const isUploading = upload.isPending && pendingCategory === cat.id;
          return (
            <div
              key={cat.id}
              className="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden"
              data-testid={`card-doc-category-${cat.id.toLowerCase().replace(/\s/g, "-")}`}
            >
              <div className="px-5 py-3 border-b border-slate-100 flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[11px] text-slate-500 truncate">{cat.hint}</p>
                </div>
                <button
                  onClick={() => triggerCategoryUpload(cat.id)}
                  disabled={upload.isPending}
                  className="inline-flex items-center gap-1.5 px-2.5 py-1.5 text-xs font-medium text-indigo-700 bg-indigo-50 hover:bg-indigo-100 border border-indigo-200 rounded-lg disabled:opacity-50 flex-none"
                  data-testid={`button-upload-category-${cat.id.toLowerCase().replace(/\s/g, "-")}`}
                >
                  {isUploading ? (
              ...
              {docs.length === 0 ? (
                <div className="px-5 py-4 text-xs text-slate-400 italic" data-testid={`empty-category-${cat.id.toLowerCase().replace(/\s/g, "-")}`}>
