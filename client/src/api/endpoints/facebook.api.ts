...
export const facebookApi = {
  getPages: async (userId: string): Promise<FacebookPagePublic[]> => {
    const { data } = await axiosClient.get<FacebookPagePublic[]>("/api/facebook/pages", { params: { userId } });
    return data;
  },

  disconnectPage: async (pageId: string, userId: string): Promise<void> => {
    await axiosClient.delete(`/api/facebook/pages/${pageId}`, { params: { userId } });
  },

  getConversations: async (pageId: string, userId: string): Promise<FbConversation[]> => {
    const { data } = await axiosClient.get<FbConversation[]>(`/api/facebook/pages/${pageId}/conversations`, {
      params: { userId },
    });
    return data;
  },
  ...
  getMessages: async (conversationId: string, pageId: string, userId: string): Promise<FbMessage[]> => {
    const { data } = await axiosClient.get<FbMessage[]>(`/api/facebook/conversations/${conversationId}/messages`, {
  ...
  sendMessage: async (pageId: string, recipientId: string, text: string, userId: string): Promise<void> => {
    await axiosClient.post(`/api/facebook/pages/${pageId}/send`, { recipientId, text }, { params: { userId } });
  },
};
