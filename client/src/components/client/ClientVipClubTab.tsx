        ...
        {pendingCommissions.length === 0 ? (
          ...
          <div className="divide-y divide-black/[0.06]">
            {pendingCommissions.map((row) => {
              ...
              const referredName = [row.referredClientFirstName, row.referredClientSurname]
                .filter(Boolean).join(" ") || row.referredName || "Client";
              ...
              return (
                ...
                >
                  <div className={`mt-0.5 flex h-7 w-7 shrink-0 items-center justify-center rounded-full ${isDue ? "bg-emerald-100" : "bg-amber-50"}`}>
                    {isDue
                    ...
                    <div className="flex flex-wrap gap-x-3 gap-y-0.5 text-[11px] text-black/45">
                      ...
                      {triggerDate && (
                        <span className="flex items-center gap-1">
                          ...
                          {isDue
                            ? "Payout trigger date passed"
                            : `Releases: ${formatDate(triggerDate)}`}
                        </span>
                      )}
                    </div>
                  </div>
