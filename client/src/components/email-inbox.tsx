...
export default function EmailInbox() {
  const { user } = useAuth();
  const { data: account = null, isLoading: accountsLoading } = useSharedEmailAccount();
  ...
  // No shared account configured
  if (!account) {
    return (
      <div className="flex items-center justify-center h-[calc(100vh-12rem)]">
        <div className="text-center space-y-2">
          <Mail className="h-10 w-10 mx-auto text-black/20 dark:text-white/20" />
          <p className="text-sm font-medium text-black/50 dark:text-white/50">No email account configured</p>
          <p className="text-xs text-black/35 dark:text-white/35">Ask your administrator to set up the shared email account.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <section
      ...
      style={{
        gridTemplateColumns: panelsCollapsed ? "1fr" : "220px 340px 1fr",
        transition: "grid-template-columns 0.25s ease",
      }}
      data-testid="section-email"
    >
      {/* Folder sidebar */}
      <Card className={`glass ringed grain flex flex-col rounded-3xl p-3 overflow-hidden transition-all duration-250 ${panelsCollapsed ? "hidden" : ""}`}>
        <Button
          onClick={() => { setComposing(true); setSelectedId(null); setComposeData({ to: "", cc: "", subject: "", body: signature }); }}
      ...
      {/* Email list */}
      <Card className={`glass ringed grain flex flex-col rounded-3xl p-0 overflow-hidden ${panelsCollapsed ? "hidden" : ""}`}>
        ...
        <AnimatePresence mode="wait">
          {composing ? (
              ...
              <div className="flex items-center justify-between border-b border-black/10 px-5 py-3 dark:border-white/10">
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="h-7 w-7 rounded-lg flex items-center justify-center text-black/40 hover:text-black hover:bg-black/5 dark:text-white/40 dark:hover:text-white dark:hover:bg-white/5 transition"
                    title={panelsCollapsed ? "Show panels" : "Hide panels"}
                  >
                    {panelsCollapsed ? <PanelLeftOpen className="h-4 w-4" /> : <PanelLeftClose className="h-4 w-4" />}
                  </button>
            ...
            <motion.div
              ...
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              className="flex flex-col items-center justify-center h-full text-black/30 dark:text-white/30 gap-3 relative"
            >
              <button
                onClick={() => setPanelsCollapsed((v) => !v)}
                className="absolute top-3 left-3 h-8 w-8 rounded-xl flex items-center justify-center text-black/30 hover:text-black hover:bg-black/5 dark:text-white/30 dark:hover:text-white dark:hover:bg-white/5 transition"
                title={panelsCollapsed ? "Show panels" : "Hide panels"}
              >
                {panelsCollapsed ? <PanelLeftOpen className="h-4 w-4" /> : <PanelLeftClose className="h-4 w-4" />}
              </button>
