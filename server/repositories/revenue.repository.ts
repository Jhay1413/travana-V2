...
export const revenueRepository = {
  /**
   * Get bookings where commission is paid in a specific month
   * Commission is paid 8 weeks (56 days) before travel date
   * So for a given month, we need bookings with travel dates 8 weeks AFTER that month
   */
  async getForwardsForMonth(year: number, month: number): Promise<{
    ...
    // Calculate travel date range (8 weeks = 56 days after the month)
  ...
  /**
   * Get detailed bookings for a specific month's forwards
   */
  ...
  /**
   * Get agent performance across all bookings
   */
  async getAgentPerformance(): Promise<Array<{
    ...
    // Get all active bookings with future travel dates
    const now = new Date();
  ...
  /**
   * Get total bookings and commission for calculating averages
   */
