...
export const opportunitiesRepository = {
  ...
  async findQuotes(filters: OpportunityFilters) {
    ...
    const rows = await db.select({ id: quote.id, transactionId: transaction.id, clientId: transaction.client_id, clientTitle: clientTable.title, clientFirstName: clientTable.firstName, clientSurname: clientTable.surename, clientPhone: clientTable.phoneNumber, agentFirstName: user.firstName, agentName: user.name, title: quote.title, status: quote.quote_status, travelDate: quote.travel_date, dateCreated: quote.date_created, salesPrice: quote.sales_price, commission: quote.package_commission, nights: quote.num_of_nights, adults: quote.adult, children: quote.child }).from(quote).innerJoin(transaction, eq(quote.transaction_id, transaction.id)).leftJoin(clientTable, eq(transaction.client_id, clientTable.id)).leftJoin(user, eq(user.id, transaction.user_id)).where(whereClause).orderBy(orderBy).limit(limit).offset(offset);
    return { rows, total: totalResult?.total || 0 };
  },
  ...
  async findBookings(filters: OpportunityFilters) {
    ...
    const rows = await db.select({ id: booking.id, transactionId: transaction.id, clientId: transaction.client_id, clientTitle: clientTable.title, clientFirstName: clientTable.firstName, clientSurname: clientTable.surename, clientPhone: clientTable.phoneNumber, agentFirstName: user.firstName, agentName: user.name, title: booking.title, status: booking.booking_status, travelDate: booking.travel_date, dateCreated: booking.date_created, salesPrice: booking.sales_price, commission: booking.package_commission, nights: booking.num_of_nights, adults: booking.adult, children: booking.child, haysRef: booking.hays_ref, supplierRef: booking.supplier_ref }).from(booking).innerJoin(transaction, eq(booking.transaction_id, transaction.id)).leftJoin(clientTable, eq(transaction.client_id, clientTable.id)).leftJoin(user, eq(user.id, transaction.user_id)).where(whereClause).orderBy(orderBy).limit(limit).offset(offset);
    return { rows, total: totalResult?.total || 0 };
  },
