...
function getCreds() {
  ...
  if (!username || !apiKey) {
    throw new Error('ClickSend is not connected. Set CLICKSEND_USERNAME, CLICKSEND_API_KEY, and CLICKSEND_SENDER as project secrets.');
  }
...
export async function sendSms({ to, body }: { to: string; body: string }): Promise<{ sid: string; status: string }> {
  ...
  const res = await fetch(`${CLICKSEND_BASE}/sms/send`, {
    ...
    body: JSON.stringify({ messages: [{ source: 'tinas-crm', from: sender, to, body }] }),
  });
  const data: any = await res.json().catch(() => ({}));
  if (!res.ok || data?.http_code !== 200) {
    const detail = data?.response_msg || data?.data?.messages?.[0]?.status || `HTTP ${res.status}`;
...
export async function pingSmsConnection(): Promise<{ connected: boolean; fromPhone?: string; balance?: string; error?: string }> {
  try {
    ...
    const res = await fetch(`${CLICKSEND_BASE}/account`, {
      headers: { Authorization: authHeader(username, apiKey), Accept: 'application/json' },
    });
    const data: any = await res.json().catch(() => ({}));
    if (!res.ok || data?.http_code !== 200) {
