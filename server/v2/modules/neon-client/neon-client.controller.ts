...
export const neonClientController = {
  ...
  getNeonClientById: asyncHandler(async (req: Request, res: Response) => {
    const client = await neonClientService.getNeonClientById(req.params.id as string);
  ...
  updateNeonClient: asyncHandler(async (req: Request, res: Response) => {
    const client = await neonClientService.updateNeonClient(req.params.id as string, req.body);
  ...
  deleteNeonClient: asyncHandler(async (req: Request, res: Response) => {
    await neonClientService.deleteNeonClient(req.params.id as string);
    res.status(204).send();
  }),
