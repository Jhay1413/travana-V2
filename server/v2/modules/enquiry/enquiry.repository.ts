...
export const enquiryTableRepository = {
  ...
  async findWithRelations(id: string) {
    ...
    const [destinations, resortList, accommodations, boardBases, airports, ports, cruiseLines, cruiseDestinations, passengerList] = await Promise.all([
      ...
      }).from(enquiry_destination).leftJoin(destination, eq(enquiry_destination.destination_id, destination.id)).leftJoin(park, eq(enquiry_destination.destination_id, park.id)).where(eq(enquiry_destination.enquiry_id, id)),
      ...
      }).from(enquiry_resorts).leftJoin(resorts, eq(enquiry_resorts.resorts_id, resorts.id)).leftJoin(lodges, eq(enquiry_resorts.resorts_id, lodges.id)).where(eq(enquiry_resorts.enquiry_id, id)),

      db.select({ enquiry_id: enquiry_accomodation.enquiry_id, accomodation_id: enquiry_accomodation.accomodation_id, accomodation_name: accomodation_list.name }).from(enquiry_accomodation).leftJoin(accomodation_list, eq(enquiry_accomodation.accomodation_id, accomodation_list.id)).where(eq(enquiry_accomodation.enquiry_id, id)),

      db.select({ enquiry_id: enquiry_board_basis.enquiry_id, board_basis_id: enquiry_board_basis.board_basis_id, board_basis_name: board_basis.type }).from(enquiry_board_basis).leftJoin(board_basis, eq(enquiry_board_basis.board_basis_id, board_basis.id)).where(eq(enquiry_board_basis.enquiry_id, id)),

      db.select({ enquiry_id: enquiry_departure_airport.enquiry_id, airport_id: enquiry_departure_airport.airport_id, airport_name: sql<string>`concat(${airport.airport_name}, ' (', ${airport.airport_code}, ')')` }).from(enquiry_departure_airport).leftJoin(airport, eq(enquiry_departure_airport.airport_id, airport.id)).where(eq(enquiry_departure_airport.enquiry_id, id)),

      db.select({ enquiry_id: enquiry_departure_port.enquiry_id, port_id: enquiry_departure_port.port_id, port_name: port.name }).from(enquiry_departure_port).leftJoin(port, eq(enquiry_departure_port.port_id, port.id)).where(eq(enquiry_departure_port.enquiry_id, id)),

      db.select({ enquiry_id: enquiry_cruise_line.enquiry_id, cruise_line_id: enquiry_cruise_line.cruise_line_id, cruise_line_name: cruise_line.name }).from(enquiry_cruise_line).leftJoin(cruise_line, eq(enquiry_cruise_line.cruise_line_id, cruise_line.id)).where(eq(enquiry_cruise_line.enquiry_id, id)),

      db.select({ enquiry_id: enquiry_cruise_destination.enquiry_id, cruise_destination_id: enquiry_cruise_destination.cruise_destination_id, cruise_destination_name: cruise_destination.name }).from(enquiry_cruise_destination).leftJoin(cruise_destination, eq(enquiry_cruise_destination.cruise_destination_id, cruise_destination.id)).where(eq(enquiry_cruise_destination.enquiry_id, id)),
