...
export async function generateWithdrawalInvoiceBuffer(data: WithdrawalInvoiceData): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    ...
    doc.fillColor(ACCENT).fontSize(22).font('Helvetica-Bold').text("Tina's Travel", 50, 50);
    doc.fillColor(GREY).fontSize(9).font('Helvetica').text('Referral Withdrawal Invoice', 50, 78);

    const invoiceNo = `INV-${data.withdrawalId.slice(0, 8).toUpperCase()}`;
    doc.fillColor(BLACK).fontSize(9).font('Helvetica-Bold').text(invoiceNo, 400, 50, { width: 145, align: 'right' });
    ...
    doc.fillColor(GREY).fontSize(8).font('Helvetica-Bold').text('BOOKING DETAILS', 56, y + 3);
    y += 22;
...
export async function uploadWithdrawalInvoice(withdrawalId: string, pdfBuffer: Buffer): Promise<string> {
...
export async function uploadWalletDebitInvoice(transactionId: string, pdfBuffer: Buffer): Promise<string> {
